...
from io import StringIO as sio # python 3 fix
import numpy as np
import warnings
#import lzf
...
def parse_header(lines):
    ...
    for ln in lines:
        ...
        if key == 'version':
            ...
            #metadata[key] = [ e for e in re.split("[, \-!?:_\t]+", value) if e ] # fix Python 3 ? no
        elif key in ('size', 'count'):
            #metadata[key] = map(int, value.split())
            metadata[key] = list(map(int, value.split())) # fix python 3
        elif key in ('width', 'height', 'points'):
            metadata[key] = int(value)
        elif key == 'viewpoint':
            #metadata[key] = map(float, value.split())
            metadata[key] = list(map(float, value.split())) # fix python 3
        elif key == 'data':
    ...
    *TODO* allow 'proper' multi-count fields.
    ...
    for f, c, t, s in zip(metadata['fields'],
        ...
        if c == 1:
            ...
            fieldnames.extend(['{}_{:04d}'.format(f, i) for i in range(c)]) # fix python 3
            typenames.extend([np_type]*c)
    #dtype = np.dtype(zip(fieldnames, typenames))
    dtype = np.dtype([*zip(fieldnames, typenames)]) # fix python 3
    return dtype
...
def point_cloud_from_fileobj(f):
    ...
    while True:
        #ln = f.readline().strip()
        ln = f.readline().strip().decode('ascii') # fix python 3
        header.append(ln)
